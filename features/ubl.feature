...
# Insurance Details flow
    ...
    And User adds Beneficiary Owner Details for entity "Myntra Corp" and applicant "Shea Test"
     #     # DMS - Upload mandatory documents before moving to Sanction Approval
    And User uploads mandatory documents and marks OSV in Documents tab
    And User selects "Move to Sanction Approval" from Application Actions and moves appForm next stage
    # QC Review
     And User reassigns application to level "L5" and user "tenjin.user@creditsaison-in.com"
     And User selects "Move to QC Review" from Application Actions and moves appForm next stage
    #QC Approve
    And User moves appForm to "QC_APPROVE" stage
    # Approve DMS ---
    And User approves the KYC checklist in Documents tab
    And User selects "Approve this Application" from Application Actions and moves appForm next stage
